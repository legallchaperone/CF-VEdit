# ADR-0004 — Teacher-forced new tokens as a B4-legal runtime overlay
...
- `overlay.attach_e2w_heads` composes onto the *loaded instance*: `[SEG_DIR]`,
  ...
  queries.
- `teacher_forced.localize_three_layer` builds the prompt's merged `inputs_embeds`
...
- **+** Validated on the real checkpoint (`spike_teacher_forced.py` and a
