...
| ② the **indirect / multi-hop** layer of the three-layer mask (the 命门) | `localization/e2w_localization/{overlay,query_tokens,planner}.py` (full: query-token `[SEG_DIR]`/`[SEG_IND]` → real direct+indirect; vanilla: stock `[SEG]`→direct, indirect empty) + `data_engine/e2w_data_engine/{davis2017_remove.py,name_objects_vlm.py}` for v0 Stage 0/1 pseudo-labels | `data_engine/tests/test_davis2017_remove.py` guards DAVIS palette-index labels, VLM/manual target names, instruction-mask alignment, integral-pair merging, grey-mask quarantine, and quadmask consistency | 🔁 original sim-dependency-graph target is superseded for v0, but the Stage 0/1 DAVIS+VOID label builder now exists (ADR-0008); Stage 2 paired sim data still not started |
