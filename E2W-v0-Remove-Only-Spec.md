...
**⚠️ 训练前置(否则无声失败)**:Stage 2 的 renderer 必须用 `model_full_load` + bf16 加载,**禁用** `model_cpu_offload_and_qfloat8`(它是 CogVideoX-Fun/`predict_v2v.py`/`VoidRendererConfig` 的默认)。float8 量化会**静默切断**反向传播——edit token 那条端到端可微链路(第 0 节的核心主张、这整个项目的立身之本)会在完全不报错的情况下断掉:loss 照常下降(seg/其他梯度还在),但 renderer 那段根本没反传,训出来的 edit token 学不到东西。与"mask 分支不可微"是同一类坑,已赶在训练开始前查明,另见 §5.10。
...
8. **`results/void/` 已经用真实 pipeline 重新跑过,不再是开放问题**:faithful 版本(真实 SAM2+Gemini quadmask;因 SAM3 被墙,用 LangSAM 代替)在 remove 六条上 edit_success 0.333、consequence 0.306、physical 0.333、preservation 0.833、quality 0.50,只有 2/6(气球、鸭子)干净通过——原来那版膨胀 mask 的数字(0.833/0.778/0.833/1.0/0.75)确认是虚高的。**这次替换混进了一个新变量**:LangSAM 代替 SAM3 不是计划内的改动,现在的 2/6 里有多少归因于换真实 mask、多少归因于换分割 backbone,没有分开。E2W 最终报告结果时,E2W 和 VOID 要用同一套 LangSAM/gemini-2.5-pro 配置跑,不能一边用换过的、一边用没换的。
9. **frozen renderer 本身可能是瓶颈,不只是 conditioning 精不精确。** faithful VOID 的两类失败——0018/0053 残影(mask 覆盖对了但没擦干净)、0048/0246 液面扰动没恢复平静(受影响区域标对了但没生成对的内容)——读起来更像 renderer 自身生成能力不够,不是 mask 或文本条件不精确。如果真是这样,E2W 的 query token 再准也救不了这两类失败,因为 renderer 和 VOID 用的是同一份冻结权重,conditioning 只能告诉它"哪里改、改成什么方向",治不了它自己生成不出干净结果。不代表项目站不住:seg 分支训练目标就是这条真实 pipeline,起点不会比 faithful VOID 差;edit token 能不能给出比 VOID 一次性文本描述更精确的物理后果信号,还是要 Stage 2 实测的开放问题。但看 Stage 2 结果时要记得:如果 E2W 在这两类失败模式上和 VOID 表现相近,大概率是撞到了同一个 renderer 天花板,不代表 conditioning 设计有问题;只有明显好于 VOID,才真正说明 conditioning 起了作用。
10. **Stage 2 renderer 配置会静默断梯度(已提前查明,非开放,记录在案)**:CogVideoX-Fun 的默认 `model_cpu_offload_and_qfloat8`(offload+float8 量化)在训练时会切断经 renderer 回传到 edit token 的梯度,且**完全不报错**——loss 仍下降(seg/其他分支梯度还在),但 edit 链路实际没学到东西。Stage 2 必须 `model_full_load`+bf16(见 §2 Stage 2 前置)。这跟第 0 节"seg 分支不可微"是同类的"无声失败",区别是它是配置导致、可避免;已在训练开始前记录,不等训完才排查。
...
- **Stage 2**:CF-VEdit remove 子集上,IP×CR 相关指标不劣于 faithful VOID(`results/void/`,physical 0.333、quality 0.50——不是早先虚高的 0.833/0.75)。因为 renderer 完全相同,提升理论上能归因于 query token 耦合方式本身,但参见第 5 节风险 9:如果 E2W 在 VOID 那两类失败模式(残影、液面扰动未恢复)上表现相近,大概率是撞到了共享的 renderer 天花板,不代表 conditioning 没用;只有明显好于 faithful VOID,才真正说明 conditioning 起了作用。
