...
`seg_dir`/`seg_ind` 的 hidden state 各自过 projection,作为 SAM2(**冻结**)的 prompt embedding,输出两个二值 mask:direct(待移除物体)、indirect(受物理影响的区域)。
...
1. **VOID 权重 license** 尚未确认,直接影响能否用于这次投稿,需要先查。
